...
await ensureLocalAuthSchema();   
 ...
 const accountId = (accountResult.rows[0] as Record<string, unknown>).id;
 console.info("Account Result:", accountResult.rows);
 console.info("Account ID:", accountId);

 const installationToken = await getInstallationToken(accountId as string);

 console.info("Installation Token created for account:", accountId);
 const userId = (userIdResult.rows[0] as Record<string, unknown>).id;
 ...
 console.info("Profile Account ID:", accountId);
